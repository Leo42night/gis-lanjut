  ...
  <div class="container">
    <nav class="d-flex flex-column my-2">
      ...
      <div class="mt-2 d-flex justify-content-between">
        <form id="poi-form" class="d-flex gap-2 flex-wrap">
          <input type="text" id="name" name="name" placeholder="POI Name" required value="<?= $data['name'] ?>">
          <input type="text" id="description" name="description" placeholder="Description" required value="<?= $data['description'] ?>">
          <input type="double" id="lat" name="lat" placeholder="Latitude" value="<?= $lat ?>">
          <input type="double" id="lng" name="lng" placeholder="Longitude" value="<?= $lng ?>">
    ...
    <script>
      ...
      document.getElementById('poi-form').addEventListener('submit', function (e) {
        ...
        console.log(`id=${id}&name=${name}&description=${description}&lat=${lat}&lng=${lng}`);
        // if (lat != 0 || lng != 0) {
        //   fetch('poi.php', {
        //     method: 'POST',
        //     headers: {
